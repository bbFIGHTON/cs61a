...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps = [string for string in paragraphs if select(string)]
    if k < len(ps):
        return ps[k]
    else:
        return ''
...
def about(subject):
    ...
    def about_checker(paragraph):
        for word in subject:
            if lower(word) in split(lower(remove_punctuation(paragraph))):
                return True
        return False
    return about_checker
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamps in tpp:
        tpp = [(timestamps[i+1] - timestamps[i]) for i in range(len(words))]
        times.append(tpp)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        fastest_player = 0
        fast_of_word = times[0][word_index]
        for player_index in player_indices:            
            if fast_of_word > times[player_index][word_index]:
                fastest_player = player_index
                fast_of_word = times[fastest_player][word_index]
        fastest_words_list[fastest_player] += [words[word_index]]
